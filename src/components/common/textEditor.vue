<template>
  <el-dialog
    ...
    <div v-if="params&&params.fields">
      <el-input v-if="'title' in params.fields" v-model="data.title" :placeholder="$t('toSave')" style="margin-bottom: 20px;">
        <el-button @click="handleSave" slot="append" icon="el-icon-success">
          {{ $t('toSave')}}
        </el-button>
      </el-input>

      <el-button v-if="!('title' in params.fields)" @click="handleSave" type="success" style="margin-bottom: 20px;" plain>
        {{ $t('toSave')}}
      </el-button>
